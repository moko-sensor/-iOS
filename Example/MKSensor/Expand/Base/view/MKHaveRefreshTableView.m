...
#import "MKRefreshGifHeader.h"
#import "MKRefreshGifFooter.h"
...
- (void)addHeader{
    ...
    self.mj_header= [MKRefreshGifHeader headerWithRefreshingBlock:^{
        weakSelf.requestType = REQUEST_REFRESH;
        weakSelf.currentPage = 1;
        [weakSelf headerBeginRefresh];
    }];
...
- (void)addFooter{
    ...
    self.mj_footer = [MKRefreshGifFooter footerWithRefreshingBlock:^{
        weakSelf.requestType = OPERATE_LOADINGMORE;
        weakSelf.currentPage++;
        [weakSelf footerBeginRefresh];
    }];
