...
static NSTimeInterval const defaultCommandTime = 6.f;
...
- (void)configWorkMode:(sensorWorkMode)workMode
              sucBlock:(void (^)(id returnData))sucBlock
           failedBlock:(void (^)(NSError *error))failedBlock{
    NSDictionary *commandDic = @{
                                 @"header":@(4003),
                                 @"mqtt_channel":(workMode == sensorWorkInGPRSMode ? @(2) : @(1)),
                                 };
    NSString *jsonString = [MKSocketAdopter convertToJsonData:commandDic];
    [self addTaskWithTaskID:socketConfigWorkModeOperation
                 jsonString:jsonString
                   sucBlock:sucBlock
                failedBlock:failedBlock];
}
